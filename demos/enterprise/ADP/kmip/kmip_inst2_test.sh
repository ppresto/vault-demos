...
MONGO_DB_DATA="${DIRECTORY}/mongodb2"
MONGO_LOG="mongodb2.log"
...
pe "vault secrets enable -path=kmip2 kmip"
echo
green "Configure KMIP to listen on default port 5696"
pe "vault write kmip2/config listen_addrs=0.0.0.0:5696"

echo
green "Read KMIP secrets engine configuration"
vault read kmip2/config
echo
yellow "Retrieve the CA and save to ca.pem"
pe "vault read -field=ca_pem kmip2/ca > ca.pem"
...
pe "vault write -f kmip2/scope/salesforce2"
vault list kmip/scope
echo
green "Create a new role, 'admin' under salesforce"
pe "vault write kmip2/scope/salesforce2/role/admin operation_all=true"
vault list kmip2/scope/salesforce2/role
#echo
...
pe "vault write -format=json kmip2/scope/salesforce2/role/admin/credential/generate format=pem_bundle | jq -r .data.certificate > client.pem"
...
p "mongod --dbpath ${MONGO_DB_DATA} --port 23456 \\
    --enableEncryption \\
