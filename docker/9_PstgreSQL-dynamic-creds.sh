...
cyan "Step 3: Configure the Postgres DB Connection and allowed_roles (creating roles next)"
tee postgres.json <<EOF
{
  "plugin_name": "postgresql-database-plugin",
  "allowed_roles": "pg-get-creds-dev",
  "connection_url": "postgresql://root:rootpassword@host.docker.internal:5432/postgres?sslmode=disable"
}
EOF
...
cyan "Step 5: Now Vault is configured.  Lets create a sample dev-team policy we will use to generate a token that can interact with Vaults new dyanmic secrets"
...
cyan "Let's request db credentials.  Every request generates a new credential !!"
pe "curl -s --header \"X-Vault-Token: $APP_TOKEN\" --request GET \\
    ${VAULT_ADDR}/v1/database/creds/pg-get-creds-dev |jq"

cyan "lets request another set of db credentials to connect to the DB and list all users (Requires psql)"
TEMP_CREDS=$(
...
pe "psql postgresql://${username}:${password}@localhost:5432/postgres?sslmode=disable -c '\du'"

cyan "Lets lookup our credentials expiration time and then extend it 1800 seconds"
pe "curl -s --header \"X-Vault-Token: $VAULT_TOKEN\" --request PUT \\
    --data '{\"lease_id\":\"${lease}\"}' \\
    ${VAULT_ADDR}/v1/sys/leases/lookup | jq"

pe "curl -s --header \"X-Vault-Token: $VAULT_TOKEN\" --request PUT \\
    --data '{\"lease_id\":\"${lease}\",\"increment\":1800}' \\
